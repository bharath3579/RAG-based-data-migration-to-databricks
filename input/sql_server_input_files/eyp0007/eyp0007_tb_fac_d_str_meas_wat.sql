CREATE TABLE [sch_anl].[eyp0007_tb_fac_d_str_meas_wat]
(
  [fec_production_day] [smalldatetime] NOT NULL,
  [id_vers_stream] [nvarchar] (4000) NOT NULL,
  [id_stream] [nvarchar] (4000) NOT NULL,
  [id_fcty_class_1] [nvarchar] (4000) NOT NULL,
  [id_col_point] [nvarchar] (4000) NOT NULL,
  [id_productionunit] [nvarchar] (4000) NOT NULL,
  [id_area] [nvarchar] (4000) NOT NULL,
  [id_operator_route] [nvarchar] (4000) NOT NULL,
  [id_product] [nvarchar] (4000) NOT NULL,
  [id_licence] [nvarchar] (4000) NOT NULL,
  [id_disposition_type] [nvarchar] (4000) NOT NULL,
  [ind_alloc_water_vol_bbl] [float],
  [ind_alloc_water_vol_m3] [float],
  [ind_alloc_water_vol_stb] [float],
  [ind_daily_net_vol_bbl] [float],
  [ind_daily_net_vol_m3] [float],
  [ind_grs_vol_wat_sm3] [float],
  [ind_grs_vol_wat_m3] [float],
  [ind_grs_vol_wat_stb] [float],
  [ind_grs_vol_wat_bbl] [float],
  [ind_grs_vol_wat_boe] [float],
  [ind_o_alloc_water_vol_stb] [float],
  [ind_o_alloc_water_vol_bbl] [float],
  [ind_o_alloc_water_vol_m3] [float],
  [ind_p_alloc_water_vol_stb] [float],
  [ind_p_alloc_water_vol_bbl] [float],
  [ind_p_alloc_water_vol_m3] [float],
  [fec_create_date] [smalldatetime],
  [fec_update_date] [smalldatetime]
)
WITH
(
	DISTRIBUTION = HASH(id_stream),
	CLUSTERED COLUMNSTORE INDEX
)
GO