...
// val sql:String = """SELECT e.id, e.name, d.dept_name, e.year, e.month, e.day
// FROM table_1 e
// JOIN table_2 d
...
// val target = """{
//         "target_object":"datahub01%env%aucdatagold.sandbox_crp0025.t_table1",
//         "target_format": "table",
//         "target_write_mode":   "overwrite",
//         "target_overwrite_mode":"static",
//         "target_dl_num_files": 0,
//         "target_format_options": {},
//         "target_dl_name_partitions": []
// }
// """
...
// val params: String = """{"orderby":"year", "dos":"1"}"""
// val auxParams: String = """{"dos":"2"}"""
...
if (continue) {
    try {
        ...
        getData.map(df_final => {
            // Check if there are fields to drop from the DataFrame
            if (dropFieldsFinal.isEmpty){
                // writeDataFrame(df_final, targetFinal)
                ...
                // writeDataFrame(df_final.drop(dropFieldsFinal:_*), targetFinal)
