...
def joinWithMst(df1: DataFrame, df2: DataFrame,fieldsPartition: Seq[String] ,df1Filter1:String, df2Filter1:String, df1Filter2:String, df2Filter2:String, selectFields: Seq[String], joinType: String): DataFrame = {
    ...
    val windowSpec = Window.partitionBy(fieldsPartition.map(df1(_)): _*).orderBy((desc(df2Filter2)))
...
val wellHoleGrp = Window.partitionBy("commercial_entity_id", "cp_col_point_id", "cp_operator_route_id", "geo_area_id","geo_field_id", 
                                                    "mur_id" , "op_area_id", "op_productionunit_id", "op_well_hookup_id", "well_hole_id", "op_fcty_1_id")

val wellHoleGrpWdw = Window.partitionBy("commercial_entity_id", "cp_col_point_id", "cp_operator_route_id", "geo_area_id","geo_field_id", 
                                                    "mur_id" , "op_area_id", "op_productionunit_id", "op_well_hookup_id", "well_hole_id", "op_fcty_1_id").orderBy(col("fec_start_date_group").asc)

val dfMstWellGrouped = dfMstWell.filter(col("well_hole_id").isNotNull).withColumn("fec_start_date_group", min("daytime").over(wellHoleGrp)).withColumn("row_number",row_number().over(wellHoleGrpWdw)).filter("row_number == 1").drop("row_number")



val dfDimWell_hole = getDataFromSource(sourcesFinal("eyp0007_tb_dim_d_well_hole")).drop("id_fcty_1").filter(col("id_vers") =!= "-99")
...
try {
    ...
    df = joinWithMst(dfDimWell_hole, dfMstWellGrouped, Seq("id_vers"),
                                        "id_well_hole", "well_hole_id",  "fec_start_date", "fec_start_date_group" ,
                                        Seq("commercial_entity_id", "op_fcty_1_id", "op_well_hookup_id","op_area_id", "op_productionunit_id", "geo_area_id" , "geo_field_id", "cp_operator_route_id", "cp_col_point_id", "mur_id") ,
                                        "left")
    df = df.withColumnRenamed("id_well_hole", "object_id").withColumnRenamed("id_vers", "id_well_hole")


    df = joinWithDim(df, dfDimComEntity, Seq("id_well_hole"), "commercial_entity_id", "id_commercial_entity", "fec_start_date", "fec_start_date", "left")
    df = df.drop("commercial_entity_id").withColumnRenamed("id_vers", "id_commercial_entity")   

    df = joinWithDimSelect(df, dfDimFctyClass1, Seq("id_well_hole"), "op_fcty_1_id", "id_facility_class_1", "fec_start_date", "fec_start_date" , Seq("id_vers", "des_facility_class_1" ), "left")
    ...
    df = joinWithDim(df, dfDimWellHookup, Seq("id_well_hole"), "op_well_hookup_id", "id_well_hookup", "fec_start_date", "fec_start_date", "left")
    df = df.drop("op_well_hookup_id").withColumnRenamed("id_vers", "id_well_hookup")

    df = joinWithDimSelect(df, dfDimArea,  Seq("id_well_hole"), "op_area_id", "id_area", "fec_start_date", "fec_start_date" , Seq("id_vers", "des_area"), "left")
    df = df.drop("op_area_id").withColumnRenamed("id_vers", "id_area").withColumn("des_area", coalesce(col("des_area"), lit("Not available")))

    df = joinWithDimSelect(df, dfDimProdUnit, Seq("id_well_hole"), "op_productionunit_id", "id_productunit", "fec_start_date", "fec_start_date" , Seq("id_vers", "des_productunit"), "left")
    ...
    df = joinWithDim(df, dfDimGeoArea, Seq("id_well_hole"), "geo_area_id", "id_geo_area", "fec_start_date", "fec_start_date", "left")
    df = df.drop("geo_area_id").withColumnRenamed("id_vers", "id_geo_area")

    df = joinWithDim(df, dfDimField, Seq("id_well_hole"), "geo_field_id", "id_field", "fec_start_date", "fec_start_date", "left")
    df = df.drop("geo_field_id").withColumnRenamed("id_vers", "id_field")

    df = joinWithDim(df, dfDimOperRoute, Seq("id_well_hole"), "cp_operator_route_id", "id_operator_route", "fec_start_date", "fec_start_date", "left")
    df = df.drop("cp_operator_route_id").withColumnRenamed("id_vers", "id_operator_route")

    df = joinWithDim(df, dfDimCollecPoint, Seq("id_well_hole"), "cp_col_point_id", "id_collection_point", "fec_start_date", "fec_start_date", "left")
    df = df.drop("cp_col_point_id").withColumnRenamed("id_vers", "id_col_point")

    df = joinWithDimCDS(df, dfDimLicence, Seq("id_well_hole"), "mur_id", "id_licence", "left")
    df = df.drop("mur_id").withColumnRenamed("id_vers", "id_licence")

    df = joinWithDimCDS(df, dfDimBasin, Seq("id_well_hole"), "id_basin", "id_basin", "left")
    df = df.drop("id_basin").withColumnRenamed("id_vers", "id_basin")

    df = df.withColumnRenamed("object_id", "id_ec_object")
           .withColumnRenamed("des_well_hole", "des_object_name")
           .drop("fec_spud_date", "fec_create_date", "fec_update_date", "val_dummy_column_partition")
    ...
    val dfFixedCols: Seq[String] = Seq("fec_start_date", "fec_end_date", "id_ec_object", "des_object_name", 
    "id_commercial_entity", "id_facility_class_1", "des_facility_class_1","id_well_hookup", "id_well_hole", 
    "id_area", "des_area","id_productionunit", "des_productunit","id_geo_area", "id_field",  
