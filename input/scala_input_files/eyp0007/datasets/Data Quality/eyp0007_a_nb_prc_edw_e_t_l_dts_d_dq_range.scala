...
def readDfFromTable(tableName: String) : DataFrame = {
    val full_table_name = getObject(s"datahub01%env%aucdatagold.eyp_operational.${if (tableName.startsWith("eyp0007_dl_")) tableName.replaceFirst("_dl_", "_tb_") else tableName}")
    val sourceMap = Map(
...
val selectFieldsSeq = Seq("fec_process","fec_production_day","id_well","id_commercial_entity","id_facility_class_1","des_facility_class_1","id_well_hookup",
"id_well_hole","id_area","des_area","id_productionunit","des_productunit","id_geo_area","id_field_cds","id_field","id_basin","id_operator_route","id_col_point",
"id_licence", "id_choke_model", "SOURCE_DATASET_NAME", "BUSINESS_TERM", "UNIT", "value", "ind_warning")
...
def selectFields (df: DataFrame): DataFrame = {
    ...
    dfTrn = dfTrn.withColumnRenamed("SOURCE_DATASET_NAME", "des_functional_dataset")
                 ...
                 .withColumnRenamed("UNIT", "des_uom")
                 .withColumnRenamed("value", "ind_value")
...
def joinWithDimSelect(df1: DataFrame, df2: DataFrame, df1Filter1:String, df2Filter1:String, selectFields: Seq[String], joinType: String): DataFrame = {


    
    val joinedDF = df1.join(df2, df1(df1Filter1) === df2(df2Filter1), joinType)

    val selectedDF = joinedDF.select((df1.columns.map(df1(_)) ++ selectFields.map(col)):_*)
    selectedDF
...
val dqFile = getDataFromSource(sourcesFinal("am_eyp0007_glb_attribute_for_dq")).filter(col("rule_type") === "RANGE")
...
try {
        rangeTables.foreach( x => {
                    ...
                    var dfTrn = getDataFromSource(sourceMap).withColumn("table_name", lit(x))
                    val trnCols: Seq[String] = dfTrn.columns.toSeq 
                    val trnFixedCols: Seq[String] = trnCols.filter(x => (x.startsWith("id_") || x=="fec_production_day" || x=="des_choke_uom" ||  x=="des_avg_gl_choke_uom" || x== "table_name")) // columnas fijas -> las que no son transpuestas o no empiezan por "ind_"
                    val trnTransposedCols: Seq[String] = trnCols.filter(_.startsWith("ind_")) // columnas transpuestas -> las que empiezan por "ind_"
                    dfTrn = toTranspose(dfTrn, trnFixedCols, trnTransposedCols)
                    ...
                    df = df.withColumn("attribute_fix", when(col("attribute").isin("ind_avg_choke_size", "ind_avg_choke_size_2"),concat_ws("_", col("attribute"), col("des_choke_uom")))
                                .when(col("attribute").isin("ind_avg_gl_choke_size"),concat_ws("_", col("attribute"), col("des_avg_gl_choke_uom")))
                                .otherwise(col("attribute")))

                    df = df.drop("des_choke_uom", "des_avg_gl_choke_uom", "attribute").withColumnRenamed("attribute_fix", "attribute")

                    df = df.withColumn("id_well_fixed", when(col("id_vers_well").isNotNull, col("id_vers_well")).otherwise(col("id_well")))
                    df = df.drop("id_vers_well", "id_well", "id_choke_model").withColumnRenamed("id_well_fixed", "id_well").withColumnRenamed("id_vers_choke_model", "id_choke_model").filter(col("value").isNotNull)

                    df = df.withColumn("id_productionunit_fixed", when(col("id_productionunit").isNotNull, col("id_productionunit")).otherwise(col("id_prod_unit")))
                    df = df.drop("id_productionunit", "id_prod_unit").withColumnRenamed("id_productionunit_fixed", "id_productionunit")

                    df = joinWithDimSelect(df, dfDimFctyClass1, "id_facility_class_1", "id_vers", Seq("des_facility_class_1" ), "left")

                    df = joinWithDimSelect(df, dfDimArea, "id_area", "id_vers", Seq("des_area"), "left")

                    df = joinWithDimSelect(df, dfDimProdUnit,"id_productionunit", "id_vers", Seq("des_productunit"), "left")
                    ...
                    val finalDfselect = selectFields(finalDf).withColumn("year",  date_format(to_date(col("fec_process")), "yyyy").cast(IntegerType))
