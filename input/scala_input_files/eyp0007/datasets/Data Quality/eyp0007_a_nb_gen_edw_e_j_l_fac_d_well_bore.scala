...
val dfDimWellBore = getDataFromSource(sourcesFinal("eyp0007_tb_dim_d_wellbore")).filter(col("id_vers") =!= "-99").drop("id_vers_well")
...
try {
    ...
    df = joinWithMst(dfDimWellBore, dfMstWell, Seq("id_vers"),
                                        ...
                                        "left")
    df = df.withColumnRenamed("id_well_bore", "object_id").withColumnRenamed("id_vers", "id_wellbore")

    
    df = joinWithDim(df, dfDimWell, Seq("id_wellbore"), "id_well", "id_well", "fec_start_date", "fec_start_date", "left")
    df = df.drop("id_well").withColumnRenamed("id_vers", "id_well")


    df = joinWithDim(df, dfDimComEntity, Seq("id_wellbore"), "commercial_entity_id", "id_commercial_entity", "fec_start_date", "fec_start_date", "left")
    df = df.drop("commercial_entity_id").withColumnRenamed("id_vers", "id_commercial_entity")
     
    df = joinWithDimSelect(df, dfDimFctyClass1, Seq("id_wellbore"), "op_fcty_1_id", "id_facility_class_1", "fec_start_date", "fec_start_date" , Seq("id_vers", "des_facility_class_1" ), "left")
    ...
    df = joinWithDim(df, dfDimWellHookup, Seq("id_wellbore"), "op_well_hookup_id", "id_well_hookup", "fec_start_date", "fec_start_date", "left")
    df = df.drop("op_well_hookup_id").withColumnRenamed("id_vers", "id_well_hookup")

    df = joinWithDim(df, dfDimWellHole, Seq("id_wellbore"), "well_hole_id", "id_well_hole", "fec_start_date", "fec_start_date", "left")
    df = df.drop("well_hole_id").withColumnRenamed("id_vers", "id_well_hole")

    df = joinWithDimSelect(df, dfDimArea,  Seq("id_wellbore"), "op_area_id", "id_area", "fec_start_date", "fec_start_date" , Seq("id_vers", "des_area"), "left")
    df = df.drop("op_area_id").withColumnRenamed("id_vers", "id_area").withColumn("des_area", coalesce(col("des_area"), lit("Not available")))

    df = joinWithDimSelect(df, dfDimProdUnit, Seq("id_wellbore"), "op_productionunit_id", "id_productunit", "fec_start_date", "fec_start_date" , Seq("id_vers", "des_productunit"), "left")
    ...
    df = joinWithDim(df, dfDimGeoArea, Seq("id_wellbore"), "geo_area_id", "id_geo_area", "fec_start_date", "fec_start_date", "left")
    df = df.drop("geo_area_id").withColumnRenamed("id_vers", "id_geo_area")

    df = joinWithDimSelectCDS(df, dfDimFieldCds, Seq("id_wellbore"), "id_field", "id_field", Seq("id_vers", "id_basin"), "left")
    df = df.drop("id_field").withColumnRenamed("id_vers", "id_field_cds")

    df = joinWithDim(df, dfDimField, Seq("id_wellbore"), "geo_field_id", "id_field", "fec_start_date", "fec_start_date", "left")
    df = df.drop("geo_field_id").withColumnRenamed("id_vers", "id_field")

    df = joinWithDimCDS(df, dfDimBasin, Seq("id_wellbore"), "id_basin", "id_basin", "left")
    df = df.drop("id_basin").withColumnRenamed("id_vers", "id_basin")

    df = joinWithDim(df, dfDimOperRoute, Seq("id_wellbore"), "cp_operator_route_id", "id_operator_route", "fec_start_date", "fec_start_date", "left")
    df = df.drop("cp_operator_route_id").withColumnRenamed("id_vers", "id_operator_route")

    df = joinWithDim(df, dfDimCollecPoint, Seq("id_wellbore"), "cp_col_point_id", "id_collection_point", "fec_start_date", "fec_start_date", "left")
    df = df.drop("cp_col_point_id").withColumnRenamed("id_vers", "id_col_point")

    df = joinWithDimCDS(df, dfDimLicence, Seq("id_wellbore"), "mur_id", "id_licence", "left")
    df = df.drop("mur_id").withColumnRenamed("id_vers", "id_licence")

    df = df.withColumnRenamed("object_id", "id_ec_object")
           .withColumnRenamed("des_well_bore", "des_object_name")
           .drop("fec_create_date", "fec_update_date", "fec_spud_date", "fec_completed_date", "val_dummy_column_partition")
    ...
    val dfFixedCols: Seq[String] = Seq("fec_start_date", "fec_end_date", "id_ec_object","id_well", "des_object_name","id_wellbore",
