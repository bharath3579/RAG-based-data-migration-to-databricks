...
val selectFieldsSeq = Seq("fec_production_day", "id_vers_stream", "id_stream", "id_fcty_class_1", "id_col_point", "id_productionunit", "id_area", "id_operator_route", "id_product", "id_licence", "id_disposition_type", "id_eqpm", "des_comments", "ind_ch4_emission_tonnesperday", "ind_co2_emission_tonnesperday", "fec_create_date", "fec_update_date")
...
val dfTransactional = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_fac_d_strm_nr_eqpm"))
...
try {
    ...
    df = joinWithMst(dfTransactional, dfMstStream, Seq("fec_production_day", "id_stream", "eqpm_id"), 
    ...
    df = joinWithDimSelect(df, dfDimStream, Seq("fec_production_day", "id_stream", "eqpm_id"), "id_stream", "id_stream", "fec_production_day", "fec_start_date", Seq("id_vers"), "left")
    df = df.withColumnRenamed("id_vers", "id_vers_stream")

    df = joinWithDim(df, dfDimFctyClass1, Seq("fec_production_day", "id_stream", "eqpm_id"), "op_fcty_1_id", "id_facility_class_1", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_fcty_1_id").withColumnRenamed("id_vers", "id_fcty_class_1")

    df = joinWithDim(df, dfDimCollecPoint, Seq("fec_production_day", "id_stream", "eqpm_id"), "cp_col_point_id", "id_collection_point", "fec_production_day", "fec_start_date", "left")
    df = df.drop("cp_col_point_id").withColumnRenamed("id_vers", "id_col_point")

    df = joinWithDim(df, dfDimProdUnit, Seq("fec_production_day", "id_stream", "eqpm_id"), "op_productionunit_id", "id_productunit", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_productionunit_id").withColumnRenamed("id_vers", "id_productionunit")

    df = joinWithDim(df, dfDimArea, Seq("fec_production_day", "id_stream", "eqpm_id"), "op_area_id", "id_area", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_area_id").withColumnRenamed("id_vers", "id_area")

    df = joinWithDim(df, dfDimOperRoute, Seq("fec_production_day", "id_stream", "eqpm_id"), "cp_operator_route_id", "id_operator_route", "fec_production_day", "fec_start_date", "left")
    df = df.drop("cp_operator_route_id").withColumnRenamed("id_vers", "id_operator_route")

    df = joinWithDim(df, dfDimLicence, Seq("fec_production_day", "id_stream", "eqpm_id"), "mur_id", "id_licence", "fec_production_day", "fec_start_date", "left")
    df = df.drop("mur_id").withColumnRenamed("id_vers", "id_licence")

    df = joinWithDim(df, dfDimProduct, Seq("fec_production_day", "id_stream", "eqpm_id"), "product_id", "id_product", "fec_production_day", "fec_start_date", "left")
    df = df.drop("product_id").withColumnRenamed("id_vers", "id_product")

    df = joinWithDim(df, dfDispType, Seq("fec_production_day", "id_stream", "eqpm_id"), "disposition_type_id", "id_disp_type", "fec_production_day", "fec_start_date", "left")
    df = df.drop("disposition_type_id").withColumnRenamed("id_vers", "id_disposition_type")

    df = joinWithDim(df, dfDimEqpm, Seq("fec_production_day", "id_stream", "eqpm_id"), "eqpm_id", "id_eqpm", "fec_production_day", "fec_start_date", "left")
    df = df.drop("disposition_type_id").withColumnRenamed("id_vers", "id_eqpm")
