...
val selectFieldsSeq = Seq("fec_production_day", "id_vers_stream", "id_stream", "id_fcty_class_1", "id_col_point", "id_productionunit", "id_area", "id_operator_route", "id_product", "id_licence", "id_disposition_type", "fec_lab_date", "fec_valid_from","fec_start_date", "des_analysis_status", "ind_analysis_no", "des_phase", "ind_rel_density", "ind_sp_grav", "fec_create_date", "fec_update_date")

val selectAttributesUnitsSeq = Seq("ind_calc_gcv_mmbtuperkscf", "ind_calc_gcv_btuperscf", "ind_calc_gcv_mjpersm3", "ind_density_lbsperkscf", "ind_density_kgpersm3", "ind_gcr_sm3persm3", "ind_gcr_kscfperstb", "ind_gcr_scfperstb", "ind_gcr_scfperbbls", "ind_gcr_mscfperbbls", "ind_gcv_mmbtuperkscf", "ind_gcv_btuperscf", "ind_gcv_mjpersm3", "ind_mol_wt_gpermol", "ind_o2_ppmv", "ind_o2_ppm", "ind_sg_mix_api")
...
def selectFields (df: DataFrame): DataFrame = {
    ...
    dfTrn = dfTrn.withColumn("ind_analysis_no", col("ind_analysis_no").cast("int"))
...
/*val setAutoMerge = paramsFinal.getOrElse("spark.databricks.delta.schema.autoMerge.enabled", "false").toString.toBoolean

if(setAutoMerge) {
    spark.conf.set("spark.databricks.delta.schema.autoMerge.enabled", setAutoMerge)
}*/

var df: DataFrame  = null

val dfTransactional = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_fac_d_str_gas_analy_uc"))
...
try {
    ...
    df = joinWithMst(dfTransactional, dfMstStream, Seq("ind_analysis_no", "id_stream", "fec_start_date"), 
    ...
    df = df.as("transactional").join(
                            df_ec_codes.filter(col("code_type") === "ANALYSIS_STATUS").drop("created_date", "last_updated_date").dropDuplicates("code_text", "bu_ec_codes").as("ec_code"), 
                            col("transactional.des_analysis_status") === col("ec_code.code") &&
                            col("transactional.id_bu_fac") === col("ec_code.bu_ec_codes"),
                            "left"
                            )
    df = df.drop("des_analysis_status", "code_type", "code", "bu_ec_codes" )
    df = df.withColumnRenamed("code_text", "des_analysis_status")

    df = joinWithDimSelect(df, dfDimStream, Seq("ind_analysis_no", "id_stream", "fec_start_date"), "id_stream", "id_stream", "fec_production_day", "fec_start_date", Seq("id_vers"), "left")
    df = df.withColumnRenamed("id_vers", "id_vers_stream")

    df = joinWithDim(df, dfDimFctyClass1, Seq("ind_analysis_no", "id_stream", "fec_start_date"), "op_fcty_1_id", "id_facility_class_1", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_fcty_1_id").withColumnRenamed("id_vers", "id_fcty_class_1")

    df = joinWithDim(df, dfDimCollecPoint, Seq("ind_analysis_no", "id_stream", "fec_start_date"), "cp_col_point_id", "id_collection_point", "fec_production_day", "fec_start_date", "left")
    df = df.drop("cp_col_point_id").withColumnRenamed("id_vers", "id_col_point")

    df = joinWithDim(df, dfDimProdUnit, Seq("ind_analysis_no", "id_stream", "fec_start_date"), "op_productionunit_id", "id_productunit", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_productionunit_id").withColumnRenamed("id_vers", "id_productionunit")

    df = joinWithDim(df, dfDimArea, Seq("ind_analysis_no", "id_stream", "fec_start_date"), "op_area_id", "id_area", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_area_id").withColumnRenamed("id_vers", "id_area")

    df = joinWithDim(df, dfDimOperRoute, Seq("ind_analysis_no", "id_stream", "fec_start_date"), "cp_operator_route_id", "id_operator_route", "fec_production_day", "fec_start_date", "left")
    df = df.drop("cp_operator_route_id").withColumnRenamed("id_vers", "id_operator_route")

    df = joinWithDim(df, dfDimLicence, Seq("ind_analysis_no", "id_stream", "fec_start_date"), "mur_id", "id_licence", "fec_production_day", "fec_start_date", "left")
    df = df.drop("mur_id").withColumnRenamed("id_vers", "id_licence")

    df = joinWithDim(df, dfDimProduct, Seq("ind_analysis_no", "id_stream", "fec_start_date"), "product_id", "id_product", "fec_production_day", "fec_start_date", "left")
    df = df.drop("product_id").withColumnRenamed("id_vers", "id_product")

    df = joinWithDim(df, dfDispType, Seq("ind_analysis_no", "id_stream", "fec_start_date"), "disposition_type_id", "id_disp_type", "fec_production_day", "fec_start_date", "left")
