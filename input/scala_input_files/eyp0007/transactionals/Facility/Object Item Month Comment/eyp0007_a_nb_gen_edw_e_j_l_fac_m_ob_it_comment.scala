...
val selectFieldsSeq = Seq("fec_production_day","id_vers_facility_class_1","id_facility_class_1",
"id_area","id_productionunit","id_comment","des_comment_even_type","des_comment_type","des_comments",
"des_report_ind","fec_start_date","fec_create_date","fec_update_date")
...
def selectFields (df: DataFrame): DataFrame = {
    ...
    dfTrn = dfTrn.withColumn("id_comment", col("id_comment").cast("double").cast("long"))
...
val dfFacObjItemComment = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_fac_m_ob_it_comment"))

val dfMstFctyClass1 = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_aux_d_fcty_class_1"))
...
try {
    ...
    df = joinWithMst(dfFacObjItemComment, dfMstFctyClass1, Seq("fec_production_day","id_comment", "id_facility_class_1",  "fec_start_date"), "id_facility_class_1", "object_id", "fec_production_day", "daytime" ,
                        Seq("op_area_id", "op_productionunit_id") , "left")

    df = df.as("transactional").join(
                            df_ec_codes.filter(col("code_type") === "COMMENT_EVENT_TYPE").drop("created_date", "last_updated_date").dropDuplicates("code_text", "bu_ec_codes").as("ec_code"), 
                            col("transactional.des_comment_even_type") === col("ec_code.code") &&
                            col("transactional.id_bu") === col("ec_code.bu_ec_codes"),
                            "left"
                            )
    df = df.drop("des_comment_even_type", "code_type", "code", "bu_ec_codes" )
    df = df.withColumnRenamed("code_text", "des_comment_even_type")

    df = df.as("transactional").join(
                            df_ec_codes.filter(col("code_type") === "COMMENTS").drop("created_date", "last_updated_date").dropDuplicates("code_text", "bu_ec_codes").as("ec_code"), 
                            col("transactional.des_comment_type") === col("ec_code.code") &&
                            col("transactional.id_bu") === col("ec_code.bu_ec_codes"),
                            "left"
                            )
    df = df.drop("des_comment_type", "code_type", "code", "bu_ec_codes" )
    df = df.withColumnRenamed("code_text", "des_comment_type")

    df = joinWithDimSelect(df, dfDimFctyClass1, Seq("fec_production_day","id_comment", "id_facility_class_1",  "fec_start_date"), "id_facility_class_1", "id_facility_class_1", "fec_production_day", "fec_start_date", Seq("id_vers"), "left")
    df = df.withColumnRenamed("id_vers", "id_vers_facility_class_1")


    df = joinWithDim(df, dfDimArea, Seq("fec_production_day","id_comment", "id_facility_class_1",  "fec_start_date"), "op_area_id", "id_area", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_area_id").withColumnRenamed("id_vers", "id_area")

    df = joinWithDim(df, dfDimProdUnit, Seq("fec_production_day","id_comment", "id_facility_class_1",  "fec_start_date"), "op_productionunit_id", "id_productunit", "fec_production_day", "fec_start_date", "left")
