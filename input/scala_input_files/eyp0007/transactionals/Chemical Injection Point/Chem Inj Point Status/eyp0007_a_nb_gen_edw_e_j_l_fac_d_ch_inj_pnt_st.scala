...
val selectFieldsSeq = Seq("fec_production_day", "id_vers_chem_inj_point", "id_chem_inj_point", "id_area", "id_collec_point", "id_fcty_class_1", 
"id_oper_route", "id_prod_unit", "id_well", "id_stream", "id_tank", "id_eqpm",  "id_chem_tank", "id_storage", "ind_chem_diff_vol",
"ind_dosage", "des_uom", "des_comments", "des_asset", "des_asset_type", "des_chem_product_name", "ind_inj_ppm","fec_create_date", "fec_update_date")

val selectAttributesUnitsSeq = Seq("ind_inj_vol_literperday", "ind_inj_vol_lperhr", "ind_inj_vol_liter")
...
val dfTransactional = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_fac_d_ch_inj_pnt_st_uc"))

val dfMstChemInjPoint = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_aux_d_chem_inj_point"))

val dfDimChemInjPoint = getDataFromSource(sourcesFinal("eyp0007_tb_dim_d_chem_inj_point"))
...
val dfDimEqpm = getDataFromSource(sourcesFinal("eyp0007_tb_dim_d_eqpm"))
val dfDimChemTank = getDataFromSource(sourcesFinal("eyp0007_tb_dim_d_chem_tank"))
...
try {
    ...
    df = joinWithMst(dfTransactional, dfMstChemInjPoint , Seq("fec_production_day", "id_chem_inj_point", "fec_update_date"), 
                                        "id_chem_inj_point", "object_id", "fec_production_day", "daytime" ,
                                        Seq( "op_fcty_1_id", "op_area_id", "op_productionunit_id", "cp_operator_route_id", "cp_col_point_id") ,
                                        "left")
  
    df = df.as("transactional").join(
                            df_ec_codes.filter(col("code_type") === "REC_DOSAGE_UOM").drop("created_date", "last_updated_date").dropDuplicates("code_text", "bu_ec_codes").as("ec_code"), 
                            col("transactional.des_uom") === col("ec_code.code") &&
                            col("transactional.id_bu_fac") === col("ec_code.bu_ec_codes"),
                            "left"
                            )
    df = df.drop("des_uom", "code_type", "code", "bu_ec_codes" )
    df = df.withColumnRenamed("code_text", "des_uom")

    df = joinWithDimSelect(df, dfDimChemInjPoint, Seq("fec_production_day", "id_chem_inj_point", "fec_update_date"), "id_chem_inj_point", "id_chem_inj_point", "fec_production_day", "fec_start_date", Seq("id_vers", "id_asset", "des_type_asset"), "left")
    df = df.withColumnRenamed("id_vers", "id_vers_chem_inj_point")



    df = joinWithDim(df, dfDimArea, Seq("fec_production_day", "id_chem_inj_point", "fec_update_date"), "op_area_id", "id_area", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_area_id").withColumnRenamed("id_vers", "id_area")

    df = joinWithDim(df, dfDimCollecPoint, Seq("fec_production_day", "id_chem_inj_point", "fec_update_date"), "cp_col_point_id", "id_collection_point", "fec_production_day", "fec_start_date", "left")
    df = df.drop("cp_col_point_id").withColumnRenamed("id_vers", "id_collec_point")

    df = joinWithDim(df, dfDimFctyClass1, Seq("fec_production_day", "id_chem_inj_point", "fec_update_date"), "op_fcty_1_id", "id_facility_class_1", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_fcty_1_id").withColumnRenamed("id_vers", "id_fcty_class_1")

    df = joinWithDim(df, dfDimOperRoute, Seq("fec_production_day", "id_chem_inj_point", "fec_update_date"), "cp_operator_route_id", "id_operator_route", "fec_production_day", "fec_start_date", "left")
    df = df.drop("cp_operator_route_id").withColumnRenamed("id_vers", "id_oper_route")

    df = joinWithDim(df, dfDimProdUnit, Seq("fec_production_day", "id_chem_inj_point", "fec_update_date"), "op_productionunit_id", "id_productunit", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_productionunit_id").withColumnRenamed("id_vers", "id_prod_unit")

    df = df.withColumn("id_well_aux", when(col("des_type_asset")==="WELL", col("id_asset")).otherwise(lit("-99")))
    df = joinWithDim(df, dfDimWell, Seq("fec_production_day", "id_chem_inj_point", "fec_update_date"), "id_well_aux", "id_well", "fec_production_day", "fec_start_date", "left")    
    df = df.drop("id_well_aux").withColumnRenamed("id_vers", "id_well")

    df = df.withColumn("id_stream_aux", when(col("des_type_asset")==="STREAM", col("id_asset")).otherwise(lit("-99")))
    df = joinWithDim(df, dfDimStream, Seq("fec_production_day", "id_chem_inj_point", "fec_update_date"), "id_stream_aux", "id_stream", "fec_production_day", "fec_start_date", "left")    
    df = df.drop("id_stream_aux").withColumnRenamed("id_vers", "id_stream")

    df = df.withColumn("id_tank_aux", when(col("des_type_asset")==="TANK", col("id_asset")).otherwise(lit("-99")))
    df = joinWithDim(df, dfDimTank, Seq("fec_production_day", "id_chem_inj_point", "fec_update_date"), "id_tank_aux", "id_tank", "fec_production_day", "fec_start_date", "left")    
    df = df.drop("id_tank_aux").withColumnRenamed("id_vers", "id_tank")

    df = df.withColumn("id_eqpm_aux", when(col("des_type_asset")==="EQPM", col("id_asset")).otherwise(lit("-99")))
    df = joinWithDim(df, dfDimEqpm, Seq("fec_production_day", "id_chem_inj_point", "fec_update_date"), "id_eqpm_aux", "id_eqpm", "fec_production_day", "fec_start_date", "left")    
    df = df.drop("id_eqpm_aux").withColumnRenamed("id_vers", "id_eqpm")

    df = df.withColumn("id_chem_tank_aux", when(col("des_type_asset")==="CHEM_TANK", col("id_asset")).otherwise(lit("-99")))
    df = joinWithDim(df, dfDimChemTank, Seq("fec_production_day", "id_chem_inj_point", "fec_update_date"), "id_chem_tank_aux", "id_chem_tank", "fec_production_day", "fec_start_date", "left")    
    df = df.drop("id_chem_tank_aux").withColumnRenamed("id_vers", "id_chem_tank")

    df = df.withColumn("id_storage_aux", when(col("des_type_asset")==="STORAGE", col("id_asset")).otherwise(lit("-99")))
    df = joinWithDim(df, dfDimStorage, Seq("fec_production_day", "id_chem_inj_point", "fec_update_date"), "id_storage_aux", "id_storage", "fec_production_day", "fec_start_date", "left")    
