...
val selectFieldsSeq = Seq("fec_production_day","id_vers_eqpm","id_eqpm","id_fcty_class_1","id_col_point","ind_calc_on_stream_hrs",
"des_class_name_type","ind_on_stream_hrs","ind_on_stream_hrs_ytd","ind_running_hrs","ind_total_on_stream_hrs","ind_total_running_hrs",
"ind_co_emission_tonnes","ind_ethane_emission_tonnes","ind_fuel_usage_tonnes","ind_hap_emissions_tonnes","ind_hcho_emission_tonnes",
"ind_metalic_emission_tonnes","ind_nox_emission_tonnes","ind_organic_emission_tonnes","ind_pm10_emission_tonnes","ind_pm25_emission_tonnes",
"ind_pmcond_emission_tonnes","ind_pmtotal_emission_tonnes","ind_sox_emission_tonnes","ind_voc_emission_tonnes","fec_create_date","fec_update_date")
...
val dfFacEqpmDayStatus = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_fac_d_eqpm_day_stat"))

val dfMstEqpm = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_aux_d_eqpm"))
...
try {
    ...
    df = joinWithMst(dfFacEqpmDayStatus, dfMstEqpm, Seq("fec_production_day","id_eqpm", "fec_update_date"), "id_eqpm", "object_id", "fec_production_day", "daytime" ,
                        Seq("op_fcty_1_id", "cp_col_point_id") , "left")


    df = joinWithDimSelect(df, dfDimEqpm, Seq("fec_production_day","id_eqpm", "fec_update_date"), "id_eqpm", "id_eqpm", "fec_production_day", "fec_start_date", Seq("id_vers"), "left")
    df = df.withColumnRenamed("id_vers", "id_vers_eqpm")


    df = joinWithDim(df, dfDimFctyClass1, Seq("fec_production_day","id_eqpm", "fec_update_date"), "op_fcty_1_id", "id_facility_class_1", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_fcty_1_id").withColumnRenamed("id_vers", "id_fcty_class_1")

    df = joinWithDim(df, dfDimCollecPoint, Seq("fec_production_day","id_eqpm", "fec_update_date"), "cp_col_point_id", "id_collection_point", "fec_production_day", "fec_start_date", "left")
