...
val selectFieldsSeq = Seq("fec_production_day", "id_vers_well", "id_well", "id_commercial_entity", "id_facility_class_1", "id_well_hookup", "id_well_hole", "id_area", "id_productionunit", "id_geo_area", "id_field_cds", "id_field",  "id_basin", "id_operator_route", "id_col_point", "id_licence", 
                           "fec_start_date", "des_comments", "fec_create_date", "fec_update_date")

val selectAttributesUnitsSeq = Seq("ind_aof_cond_rate_stbperday", "ind_aof_cond_rate_sm3perday", "ind_aof_gas_rate_mscfperday", "ind_aof_gas_rate_kscfperday", "ind_aof_gas_rate_sm3perday", "ind_aof_oil_rate_stbperday", 
"ind_aof_oil_rate_sm3perday", "ind_aof_wat_inj_rate_stbperday", "ind_aof_wat_inj_rate_sm3perday", "ind_aof_water_rate_stbperday", "ind_aof_water_rate_sm3perday", "ind_cond_rate_stbperday", "ind_cond_rate_sm3perday", 
"ind_gas_inj_rate_mscfperday", "ind_gas_inj_rate_kscfperday", "ind_gas_inj_rate_sm3perday", "ind_gas_rate_mscfperday", "ind_gas_rate_kscfperday", "ind_gas_rate_sm3perday", "ind_oil_rate_stbperday", "ind_oil_rate_sm3perday", 
"ind_wat_inj_rate_stbperday", "ind_wat_inj_rate_sm3perday", "ind_water_rate_stbperday", "ind_water_rate_sm3perday"
)
...
def replicateDataUntilNextDate(df: org.apache.spark.sql.DataFrame, dateColumn: String, dateColumn2: String, idColumn: String): org.apache.spark.sql.DataFrame = {


  val windowSpec = Window.partitionBy(idColumn).orderBy(asc(dateColumn), asc(dateColumn2))
  val dfWithNextDate = df.withColumn("next_date",
                     coalesce(lead(col(dateColumn), 1).over(windowSpec), 
                     date_add(col(dateColumn), 1)))

  val dfWithNextDate2 = dfWithNextDate.withColumn("next_date", when(col(dateColumn) === col("next_date"), date_add(col("next_date"), 1))
                                                                .otherwise(col("next_date")))
  val dfWithDateRange = dfWithNextDate2
    .withColumn(dateColumn, col(dateColumn).cast(DateType))
    .withColumn("next_date", col("next_date").cast(DateType))
    .withColumn("date_range", expr(s"sequence($dateColumn, next_date - INTERVAL 1 DAY, INTERVAL 1 DAY)"))
    .withColumn("replicated_date", explode(col("date_range")))

  dfWithDateRange.withColumn(dateColumn, col("replicated_date").cast(TimestampType)).drop("next_date", "date_range", "replicated_date")
}


def replicateDataUntilNextDate2(df: org.apache.spark.sql.DataFrame, dateColumn: String, dateColumn2: String, idColumn: String): org.apache.spark.sql.DataFrame = {

  val actualDate = java.time.LocalDate.now
  val windowSpec = Window.partitionBy(idColumn).orderBy(asc(dateColumn), asc(dateColumn2))
  val dfWithNextDate = df.filter(col(dateColumn) <= actualDate)
...
val dfTransactional = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_fac_d_well_plan_oth_uc"))
...
try {
    ...
    df = joinWithMst(dfTransactional, dfMstWell,Seq("fec_production_day", "id_well", "fec_start_date"), 
    ...
    df = replicateDataUntilNextDate2(df, "fec_production_day", "fec_start_date", "id_well")
