...
val selectFieldsSeq = Seq("fec_production_day", "id_vers_well", "id_well", "id_commercial_entity", "id_facility_class_1", "id_well_hookup", "id_well_hole", "id_area", "id_productionunit", "id_geo_area", "id_field_cds", "id_field", "id_basin", "id_operator_route", "id_col_point", "id_licence", "fec_end_date", "des_type", "fec_parent_start_date", "id_parent_well", "ind_parent_no", "ind_event_no", "des_event_reason", "des_event_category", "des_event_system", "des_root_cause", "des_scheduled", "fec_create_date", "fec_update_date")

val selectAttributesUnitsSeq = Seq("ind_cond_event_loss_stb", "ind_cond_event_loss_sm3", "ind_cond_loss_volume_stb", "ind_cond_loss_volume_sm3", "ind_gas_event_loss_mscf", "ind_gas_event_loss_kscf", "ind_gas_event_loss_sm3", "ind_gas_loss_volume_mscf", "ind_gas_loss_volume_kscf", "ind_gas_loss_volume_sm3", "ind_oil_event_loss_stb", "ind_oil_event_loss_sm3", "ind_oil_event_loss_bbl", "ind_oil_event_loss_boe", "ind_oil_loss_volume_stb", "ind_oil_loss_volume_sm3", "ind_oil_loss_volume_bbl", "ind_oil_loss_volume_boe")
...
val dfTransactional = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_fac_d_well_deferm_uc"))
...
try {
    ...
    df = joinWithMst(dfTransactional, dfMstWell,Seq("fec_production_day", "id_well", "ind_event_no"), 
    ...
    df = df.as("transactional").join(
                            df_ec_codes.filter(col("code_type") === "DEFER_CAUSE_CAT").drop("created_date", "last_updated_date").dropDuplicates("code_text", "bu_ec_codes").as("ec_code"), 
                            col("transactional.des_event_reason") === col("ec_code.code") &&
                            col("transactional.id_bu_fac") === col("ec_code.bu_ec_codes"),
                            "left"
                            )
    df = df.drop("des_event_reason", "code_type", "code", "bu_ec_codes" )
    df = df.withColumnRenamed("code_text", "des_event_reason")

    df = df.as("transactional").join(
                            df_ec_codes.filter(col("code_type") === "DEFER_CAUSE_GRP").drop("created_date", "last_updated_date").dropDuplicates("code_text", "bu_ec_codes").as("ec_code"), 
                            col("transactional.des_event_category") === col("ec_code.code") &&
                            col("transactional.id_bu_fac") === col("ec_code.bu_ec_codes"),
                            "left"
                            )
    df = df.drop("des_event_category", "code_type", "code", "bu_ec_codes" )
    df = df.withColumnRenamed("code_text", "des_event_category")

    df = df.as("transactional").join(
                            df_ec_codes.filter(col("code_type") === "DEFER_SYS_GRP").drop("created_date", "last_updated_date").dropDuplicates("code_text", "bu_ec_codes").as("ec_code"), 
                            col("transactional.des_event_system") === col("ec_code.code") &&
                            col("transactional.id_bu_fac") === col("ec_code.bu_ec_codes"),
                            "left"
                            )
    df = df.drop("des_event_system", "code_type", "code", "bu_ec_codes" )
    df = df.withColumnRenamed("code_text", "des_event_system")

    df = df.as("transactional").join(
                            df_ec_codes.filter(col("code_type") === "DEFER_ROOT_CAUSE").drop("created_date", "last_updated_date").dropDuplicates("code_text", "bu_ec_codes").as("ec_code"), 
    ...
    df = joinWithDimSelect(df, dfDimWell, Seq("fec_production_day", "id_well", "ind_event_no"), "id_well", "id_well", "fec_production_day", "fec_start_date", Seq("id_vers", "id_field_cds"), "left")
    df = df.withColumnRenamed("id_vers", "id_vers_well")

    df = joinWithDim(df, dfDimComEntity, Seq("fec_production_day", "id_well", "ind_event_no"), "commercial_entity_id", "id_commercial_entity", "fec_production_day", "fec_start_date", "left")
    df = df.drop("commercial_entity_id").withColumnRenamed("id_vers", "id_commercial_entity")
    

    df = joinWithDim(df, dfDimFctyClass1, Seq("fec_production_day", "id_well", "ind_event_no"), "op_fcty_1_id", "id_facility_class_1", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_fcty_1_id").withColumnRenamed("id_vers", "id_facility_class_1")

    df = joinWithDim(df, dfDimWellHookup, Seq("fec_production_day", "id_well", "ind_event_no"), "op_well_hookup_id", "id_well_hookup", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_well_hookup_id").withColumnRenamed("id_vers", "id_well_hookup")

    df = joinWithDim(df, dfDimWellHole, Seq("fec_production_day", "id_well", "ind_event_no"), "well_hole_id", "id_well_hole", "fec_production_day", "fec_start_date", "left")
    df = df.drop("well_hole_id").withColumnRenamed("id_vers", "id_well_hole")

    df = joinWithDim(df, dfDimArea, Seq("fec_production_day", "id_well", "ind_event_no"), "op_area_id", "id_area", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_area_id").withColumnRenamed("id_vers", "id_area")

    df = joinWithDim(df, dfDimProdUnit, Seq("fec_production_day", "id_well", "ind_event_no"), "op_productionunit_id", "id_productunit", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_productionunit_id").withColumnRenamed("id_vers", "id_productionunit")

    df = joinWithDim(df, dfDimGeoArea, Seq("fec_production_day", "id_well", "ind_event_no"), "geo_area_id", "id_geo_area", "fec_production_day", "fec_start_date", "left")
    df = df.drop("geo_area_id").withColumnRenamed("id_vers", "id_geo_area")

    df = joinWithDimSelect(df, dfDimFieldCds, Seq("fec_production_day", "id_well", "ind_event_no"), "id_field_cds", "id_field", "fec_production_day", "fec_update_date", Seq("id_vers", "id_basin"), "left")
    df = df.drop("id_field_cds").withColumnRenamed("id_vers", "id_field_cds")

    df = joinWithDim(df, dfDimField, Seq("fec_production_day", "id_well", "ind_event_no"), "geo_field_id", "id_field", "fec_production_day", "fec_start_date", "left")
    df = df.drop("geo_field_id").withColumnRenamed("id_vers", "id_field")

    df = joinWithDim(df, dfDimBasin, Seq("fec_production_day", "id_well", "ind_event_no"), "id_basin", "id_basin", "fec_production_day", "fec_update_date", "left")
    df = df.drop("id_basin").withColumnRenamed("id_vers", "id_basin")

    df = joinWithDim(df, dfDimOperRoute, Seq("fec_production_day", "id_well", "ind_event_no"), "cp_operator_route_id", "id_operator_route", "fec_production_day", "fec_start_date", "left")
    df = df.drop("cp_operator_route_id").withColumnRenamed("id_vers", "id_operator_route")

    df = joinWithDim(df, dfDimCollecPoint, Seq("fec_production_day", "id_well", "ind_event_no"), "cp_col_point_id", "id_collection_point", "fec_production_day", "fec_start_date", "left")
    df = df.drop("cp_col_point_id").withColumnRenamed("id_vers", "id_col_point")

    df = joinWithDim(df, dfDimLicence, Seq("fec_production_day", "id_well", "ind_event_no"), "mur_id", "id_licence", "fec_production_day", "fec_start_date", "left")
