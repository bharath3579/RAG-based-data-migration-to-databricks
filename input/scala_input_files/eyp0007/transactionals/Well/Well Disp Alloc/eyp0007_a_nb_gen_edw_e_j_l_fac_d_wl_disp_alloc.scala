...
val dfFacWellGasAn = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_fac_d_wl_disp_alloc"))
...
try {
    ...
    df = joinWithDimSelect(df, dfDimDispType, Seq("fec_production_day","id_well", "fec_update_date"), "cod_disposition_type", "id_disp_type", "fec_production_day", "fec_start_date", Seq("id_vers", "id_product"), "left")
    df = df.withColumnRenamed("id_vers", "id_disposition_type")

    df = joinWithDim(df, dfDimProduct, Seq("fec_production_day","id_well", "fec_update_date"), "id_product", "id_product", "fec_production_day", "fec_start_date", "left")
    df = df.drop("id_product").withColumnRenamed("id_vers", "id_product")
