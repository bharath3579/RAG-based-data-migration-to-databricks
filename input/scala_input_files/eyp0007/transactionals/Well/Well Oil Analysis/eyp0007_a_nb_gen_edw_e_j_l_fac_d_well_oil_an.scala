...
val selectFieldsSeq = Seq("fec_production_day", "id_vers_well", "id_well", "id_commercial_entity", "id_facility_class_1", "id_well_hookup", "id_well_hole", "id_area", "id_productionunit", "id_geo_area", "id_field_cds", "id_field",  "id_basin", "id_operator_route", "id_col_point", "id_licence", 
                            "fec_start_date", "fec_lab_daytime", "fec_valid_from", "ind_analysis_no", "des_analysis_status", "des_color", "des_phase", "ind_api", "fec_create_date", "fec_update_date")

val selectAttributesUnitsSeq = Seq("ind_asphaltene_ppmv", "ind_asphaltene_ppm", "ind_bsw_wt_frac", "ind_bsw_wt_pct", "ind_density_lbsperstb", "ind_density_gperscm3",
                                "ind_density_tonnesperstb", "ind_density_kgpersm3", "ind_flash_gas_factor_scfperstb", "ind_free_water_frac", "ind_free_water_pct", "ind_gcv_mmbtuperkscf",
                                 "ind_gcv_btuperscf", "ind_gcv_mjpersm3", "ind_mol_wt_gpermol", "ind_pto_escu_f", "ind_pto_escu_c", "ind_salt_ppmv",
                                 "ind_salt_ppm", "ind_sand_rate_frac", "ind_sand_rate_pct", "ind_sp_grav_sg", "ind_sulfate_ppmv", "ind_sulfate_ppm",
                                 "ind_wat_density_kgpersm3", "ind_wat_density_gperscm3", "ind_wat_density_lbsperstb"
                                )
...
def selectFields (df: DataFrame): DataFrame = {
    ...
    dfTrn = dfTrn.withColumn("ind_analysis_no", col("ind_analysis_no").cast("bigint"))
...
val dfTransactional = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_fac_d_well_oil_an_uc"))
...
val dfEcCodes = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_aux_d_ec_codes")).withColumnRenamed("id_bu", "bu_ec_codes")
...
try {
    ...
    df = df.join(
                dfEcCodes.filter(col("code_type") === "ANALYSIS_STATUS").drop("created_date", "last_updated_date").dropDuplicates("code_text", "bu_ec_codes"), 
                df("des_analysis_status") === col("code") && df("id_bu_fac") === col("bu_ec_codes"),
                "left"
                )
