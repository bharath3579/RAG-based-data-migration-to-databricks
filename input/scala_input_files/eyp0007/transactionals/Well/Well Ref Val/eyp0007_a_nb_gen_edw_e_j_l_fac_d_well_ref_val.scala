...
val selectFieldsSeq = Seq("fec_production_day", "id_vers_well", "id_well", "id_commercial_entity", "id_facility_class_1", "id_well_hookup", "id_well_hole", "id_area", "id_productionunit", "id_geo_area", "id_field_cds", "id_field", "id_basin",
                        "id_operator_route", "id_col_point", "id_licence", "des_class_name", "ind_z_well_pot_class","fec_create_date", "fec_update_date")
...
val dfFacWellEvent = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_fac_d_well_ref_val"))
...
try {
    ...
    df = joinWithMst(dfFacWellEvent, dfMstWell, Seq("fec_production_day","id_well", "fec_update_date"), "id_well", "object_id", "fec_production_day", "daytime" ,
    ...
    df = df.as("transactional").join(
                            df_ec_codes.filter(col("code_type") === "Z_WELL_POT_CLASS").drop("created_date", "last_updated_date").dropDuplicates("code_text", "bu_ec_codes").as("ec_code"), 
                            col("transactional.ind_z_well_pot_class") === col("ec_code.code") &&
                            col("transactional.id_bu") === col("ec_code.bu_ec_codes"),
                            "left"
                            )
    df = df.drop("ind_z_well_pot_class", "code_type", "code", "bu_ec_codes" )
    df = df.withColumnRenamed("code_text", "ind_z_well_pot_class")
