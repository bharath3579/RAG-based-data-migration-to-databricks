...
val selectFieldsSeq = Seq("fec_production_day", "id_vers_well", "id_well", "id_commercial_entity", "id_facility_class_1", "id_well_hookup", "id_well_hole", "id_area", "id_productionunit", "id_geo_area", "id_field_cds", "id_field", "id_basin", "id_operator_route", "id_col_point", "id_licence", "fec_end_date", "fec_valid_from", "ind_duration", "des_status", "ind_result_no", "fec_create_date", "fec_update_date", "id_bu_fac")
val selectAttributesUnitsSeq = Seq("ind_gas_rate_adj_mscfperday", "ind_gas_rate_adj_kscfperday", "ind_gas_rate_adj_sm3perday", "ind_net_cond_rate_adj_stbperday", "ind_net_cond_rate_adj_sm3perday", "ind_net_oil_rate_adj_stbperday", "ind_net_oil_rate_adj_sm3perday", "ind_tot_water_rate_adj_bblperday", "ind_tot_water_rate_adj_m3perday", "ind_si_bhp_psig", "ind_si_bhp_barg", "ind_si_whp_psig", "ind_si_whp_barg")
...
val dfTransactional = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_fac_d_wl_prod_res_uc"))
...
try {
    ...
    df = joinWithMst(dfTransactional, dfMstWell,Seq("fec_production_day", "id_well", "ind_result_no"), 
    ...
    df = df.as("transactional").join(
                            df_ec_codes.filter(col("code_type") === "WELL_TEST_STATUS").drop("created_date", "last_updated_date").dropDuplicates("code_text", "bu_ec_codes").as("ec_code"), 
                            col("transactional.des_status") === col("ec_code.code") &&
                            col("transactional.id_bu_fac") === col("ec_code.bu_ec_codes"),
                            "left"
                            )
    df = df.drop("des_status", "code_type", "code", "bu_ec_codes" )
    df = df.withColumnRenamed("code_text", "des_status")

    df = joinWithDimSelect(df, dfDimWell, Seq("fec_production_day", "id_well", "ind_result_no"), "id_well", "id_well", "fec_production_day", "fec_start_date", Seq("id_vers", "id_field_cds"), "left")
    df = df.withColumnRenamed("id_vers", "id_vers_well")



    df = joinWithDim(df, dfDimComEntity, Seq("fec_production_day", "id_well", "ind_result_no"), "commercial_entity_id", "id_commercial_entity", "fec_production_day", "fec_start_date", "left")
    df = df.drop("commercial_entity_id").withColumnRenamed("id_vers", "id_commercial_entity")
    

    df = joinWithDim(df, dfDimFctyClass1, Seq("fec_production_day", "id_well", "ind_result_no"), "op_fcty_1_id", "id_facility_class_1", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_fcty_1_id").withColumnRenamed("id_vers", "id_facility_class_1")

    df = joinWithDim(df, dfDimWellHookup, Seq("fec_production_day", "id_well", "ind_result_no"), "op_well_hookup_id", "id_well_hookup", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_well_hookup_id").withColumnRenamed("id_vers", "id_well_hookup")

    df = joinWithDim(df, dfDimWellHole, Seq("fec_production_day", "id_well", "ind_result_no"), "well_hole_id", "id_well_hole", "fec_production_day", "fec_start_date", "left")
    df = df.drop("well_hole_id").withColumnRenamed("id_vers", "id_well_hole")

    df = joinWithDim(df, dfDimArea, Seq("fec_production_day", "id_well", "ind_result_no"), "op_area_id", "id_area", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_area_id").withColumnRenamed("id_vers", "id_area")

    df = joinWithDim(df, dfDimProdUnit, Seq("fec_production_day", "id_well", "ind_result_no"), "op_productionunit_id", "id_productunit", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_productionunit_id").withColumnRenamed("id_vers", "id_productionunit")

    df = joinWithDim(df, dfDimGeoArea, Seq("fec_production_day", "id_well", "ind_result_no"), "geo_area_id", "id_geo_area", "fec_production_day", "fec_start_date", "left")
    df = df.drop("geo_area_id").withColumnRenamed("id_vers", "id_geo_area")

    df = joinWithDimSelect(df, dfDimFieldCds, Seq("fec_production_day", "id_well", "ind_result_no"), "id_field_cds", "id_field", "fec_production_day", "fec_update_date", Seq("id_vers", "id_basin"), "left")
    df = df.drop("id_field_cds").withColumnRenamed("id_vers", "id_field_cds")

    df = joinWithDim(df, dfDimField, Seq("fec_production_day", "id_well", "ind_result_no"), "geo_field_id", "id_field", "fec_production_day", "fec_start_date", "left")
    df = df.drop("geo_field_id").withColumnRenamed("id_vers", "id_field")

    df = joinWithDim(df, dfDimBasin, Seq("fec_production_day", "id_well", "ind_result_no"), "id_basin", "id_basin", "fec_production_day", "fec_update_date", "left")
    df = df.drop("id_basin").withColumnRenamed("id_vers", "id_basin")

    df = joinWithDim(df, dfDimOperRoute, Seq("fec_production_day", "id_well", "ind_result_no"), "cp_operator_route_id", "id_operator_route", "fec_production_day", "fec_start_date", "left")
    df = df.drop("cp_operator_route_id").withColumnRenamed("id_vers", "id_operator_route")

    df = joinWithDim(df, dfDimCollecPoint, Seq("fec_production_day", "id_well", "ind_result_no"), "cp_col_point_id", "id_collection_point", "fec_production_day", "fec_start_date", "left")
    df = df.drop("cp_col_point_id").withColumnRenamed("id_vers", "id_col_point")

    df = joinWithDim(df, dfDimLicence, Seq("fec_production_day", "id_well", "ind_result_no"), "mur_id", "id_licence", "fec_production_day", "fec_start_date", "left")
