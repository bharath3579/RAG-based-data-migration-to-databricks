...
val selectFieldsSeq = Seq("fec_production_day", "id_vers_choke_model", "id_choke_model", "id_area", "id_facility_class_1", "id_prod_unit", "ind_cpd_qty_boe", "ind_empp_qty_boe", 
                            "ind_epp_qty_boe", "ind_mpp_qty_boe", "ind_well_potential_boe", "ind_min_choke_boe", "des_min_choke", "fec_create_date","fec_update_date")
...
def breakDownToDaily(df: org.apache.spark.sql.DataFrame, dateColumn: String): org.apache.spark.sql.DataFrame = {
  ...
  val breakDownDfFinal = breakDownDf.withColumn(dateColumn, to_timestamp(date_add(col(dateColumn), col("day") - 1)))
...
val dfFacChokeModMpp = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_fac_m_choke_mod_mpp"))

val dfMstChokeModel = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_aux_d_choke_model"))

val dfDimChokeModel = getDataFromSource(sourcesFinal("eyp0007_tb_dim_d_choke_model"))
...
try {
    ...
    df = joinWithMst(dfFacChokeModMpp, dfMstChokeModel, Seq("fec_production_day","id_choke_model", "fec_update_date"), "id_choke_model", "object_id", "fec_production_day", "daytime" ,
                        Seq("op_area_id", "op_fcty_1_id", "op_productionunit_id") , "left")



    df = joinWithDimSelect(df, dfDimChokeModel, Seq("fec_production_day","id_choke_model", "fec_update_date"), "id_choke_model", "id_choke_model", "fec_production_day", "fec_start_date", Seq("id_vers", "des_choke_model"), "left")
    df = df.withColumnRenamed("id_vers", "id_vers_choke_model")

    df = joinWithDim(df, dfDimArea, Seq("fec_production_day","id_choke_model", "fec_update_date"), "op_area_id", "id_area", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_area_id").withColumnRenamed("id_vers", "id_area")

    df = joinWithDim(df, dfDimFctyClass1, Seq("fec_production_day","id_choke_model", "fec_update_date"), "op_fcty_1_id", "id_facility_class_1", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_fcty_1_id").withColumnRenamed("id_vers", "id_facility_class_1")

    df = joinWithDim(df, dfDimProdUnit, Seq("fec_production_day","id_choke_model", "fec_update_date"), "op_productionunit_id", "id_productunit", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_productionunit_id").withColumnRenamed("id_vers", "id_prod_unit")

    val windowSpec = Window.partitionBy("id_facility_class_1", "fec_production_day")

    df = df.withColumn("ind_well_potential_boe", 
            when(col("id_facility_class_1") === "-99", lit(null))
                .otherwise(first(when(lower(col("des_choke_model")).contains("well"), col("ind_mpp_qty_boe")), ignoreNulls = true).over(windowSpec))
                .cast("double")
            )
    df = df.withColumn("ind_min_choke_boe", 
            when(col("id_facility_class_1") === "-99", lit(null))
                .otherwise(min("ind_mpp_qty_boe").over(windowSpec))
                .cast("double")
            )

    val windowSpecOrdered = Window.partitionBy("id_facility_class_1", "fec_production_day").orderBy(col("ind_mpp_qty_boe").asc_nulls_last)

    df = df.withColumn("des_min_choke", 
            when(col("id_facility_class_1") === "-99", lit(null))
                .otherwise(first("des_choke_model").over(windowSpecOrdered))
            )

    df = breakDownToDaily(df, "fec_production_day")
    df = selectFields(df)
