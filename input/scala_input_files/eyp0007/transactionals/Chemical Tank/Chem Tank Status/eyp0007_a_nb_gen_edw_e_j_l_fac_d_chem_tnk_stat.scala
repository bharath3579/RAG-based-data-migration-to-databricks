...
val selectFieldsSeq = Seq("fec_production_day","id_vers_chem_tank","id_chem_tank","id_area","id_collec_point","id_fcty_class_1","id_oper_route",
"id_prod_unit","id_chem_product","des_comments","des_product_type","des_tank_uom","ind_closing","ind_consumed","ind_filled","ind_opening",
...
def joinWithMst(df1: DataFrame, df2: DataFrame, fieldsPartition: Seq[String], df1Filter1:String, df2Filter1:String, df1Filter2:String, df2Filter2:String, selectFields: Seq[String], joinType: String): DataFrame = {
    ...
    val windowSpec = Window.partitionBy(fieldsPartition.map(df1(_)): _*).orderBy(df2(df2Filter2).desc)
...
val dfFacChemTankStatus = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_fac_d_chem_tnk_stat"))

val dfMstChemTank = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_aux_d_chem_tank"))
...
val dfDimChemProduct = getDataFromSource(sourcesFinal("eyp0007_tb_dim_d_chem_prod"))
val dfDimChemTankProduct = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_aux_d_chem_tank_product"))
...
try {
    ...
    val dfJoinMstChemTank = joinWithMst(dfMstChemTank, dfDimChemTankProduct, Seq("object_id","daytime", "last_updated_date"), "object_id", "object_id", "daytime", "daytime" ,
            Seq("chem_product_id") , "left")

    df = joinWithMst(dfFacChemTankStatus, dfJoinMstChemTank, Seq("fec_production_day","id_chem_tank", "fec_update_date"), "id_chem_tank", "object_id", "fec_production_day", "daytime" ,
                        Seq("op_fcty_1_id", "op_area_id", "op_productionunit_id", "cp_operator_route_id", "cp_col_point_id", "chem_product_id") , "left")

    df = df.as("transactional").join(
                            df_ec_codes.filter(col("code_type") === "CHEM_PRODUCT_TYPE").drop("created_date", "last_updated_date").dropDuplicates("code_text", "bu_ec_codes").as("ec_code"), 
                            col("transactional.des_product_type") === col("ec_code.code") &&
                            col("transactional.id_bu") === col("ec_code.bu_ec_codes"),
                            "left"
                            )
    df = df.drop("des_product_type", "code_type", "code", "bu_ec_codes" )
    df = df.withColumnRenamed("code_text", "des_product_type")

    df = df.as("transactional").join(
                            df_ec_codes.filter(col("code_type") === "CHEM_UNIT").drop("created_date", "last_updated_date").dropDuplicates("code_text", "bu_ec_codes").as("ec_code"), 
                            col("transactional.des_tank_uom") === col("ec_code.code") &&
                            col("transactional.id_bu") === col("ec_code.bu_ec_codes"),
                            "left"
                            )
    df = df.drop("des_tank_uom", "code_type", "code", "bu_ec_codes" )
    df = df.withColumnRenamed("code_text", "des_tank_uom")

    df = joinWithDimSelect(df, dfDimChemTank, Seq("fec_production_day","id_chem_tank", "fec_update_date"), "id_chem_tank", "id_chem_tank", "fec_production_day", "fec_start_date", Seq("id_vers"), "left")
    df = df.withColumnRenamed("id_vers", "id_vers_chem_tank")


    df = joinWithDim(df, dfDimArea, Seq("fec_production_day","id_chem_tank", "fec_update_date"), "op_area_id", "id_area", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_area_id").withColumnRenamed("id_vers", "id_area")

    df = joinWithDim(df, dfDimCollecPoint, Seq("fec_production_day","id_chem_tank", "fec_update_date"), "cp_col_point_id", "id_collection_point", "fec_production_day", "fec_start_date", "left")
    df = df.drop("cp_col_point_id").withColumnRenamed("id_vers", "id_collec_point")

    df = joinWithDim(df, dfDimFctyClass1, Seq("fec_production_day","id_chem_tank", "fec_update_date"), "op_fcty_1_id", "id_facility_class_1", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_fcty_1_id").withColumnRenamed("id_vers", "id_fcty_class_1")

    df = joinWithDim(df, dfDimOperRoute, Seq("fec_production_day","id_chem_tank", "fec_update_date"), "cp_operator_route_id", "id_operator_route", "fec_production_day", "fec_start_date", "left")
    df = df.drop("cp_operator_route_id").withColumnRenamed("id_vers", "id_oper_route")

    df = joinWithDim(df, dfDimProdUnit, Seq("fec_production_day","id_chem_tank", "fec_update_date"), "op_productionunit_id", "id_productunit", "fec_production_day", "fec_start_date", "left")
    df = df.drop("op_productionunit_id").withColumnRenamed("id_vers", "id_prod_unit")

    df = joinWithDim(df, dfDimChemProduct, Seq("fec_production_day","id_chem_tank", "fec_update_date"), "chem_product_id", "id_chem_product", "fec_production_day", "fec_start_date", "left")
    df = df.drop("chem_product_id").withColumnRenamed("id_vers", "id_chem_product")
