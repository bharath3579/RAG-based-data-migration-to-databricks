...
val order = Seq("id_vers", "id_chem_inj_point", "des_chem_inj_point", "fec_start_date", "fec_end_date", "id_asset", "des_type_asset", "des_uom", "ind_dosage", "id_col_point", "id_fcty_1", "fec_create_date", "fec_update_date")
...
def transformationDfDim (df: DataFrame): DataFrame = {
    ...
    dfTrn = dfTrn.withColumnRenamed("object_id", "id_chem_inj_point")
                    .withColumnRenamed("name", "des_chem_inj_point")
                    ...
                    .withColumnRenamed("asset_id", "id_asset")
                    .withColumnRenamed("asset_type", "des_type_asset")
                    .withColumnRenamed("uom", "des_uom")
                    .withColumnRenamed("dosage", "ind_dosage")
...
try {
    ...
    df = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_aux_d_chem_inj_point"))

    df = transformationDfDim(df)
    df = addRowDummy(df)

    // writeDataFramePartitionDummy(df, targetFinal)
