...
val order = Seq("id_vers", "id_field", "des_field", "fec_start_date", "fec_end_date", "cod_master_sys", "des_master_sys", "des_field_long", "id_geo_area", "fec_create_date", "fec_update_date")

val rowDummy = Row("-99", "-99", "Not available", java.sql.Timestamp.valueOf("1900-01-01 00:00:00"), null, null, null, null, null, null, null)

def transformationDfDimField (df: DataFrame): DataFrame = {
    ...
    dfTrn = dfTrn.withColumnRenamed("object_id", "id_field")
                .withColumnRenamed("name", "des_field")
                .withColumnRenamed("daytime", "fec_start_date")
                .withColumnRenamed("end_date", "fec_end_date")
                .withColumnRenamed("master_sys_code", "cod_master_sys")
                .withColumnRenamed("master_sys_name", "des_master_sys")
                .withColumnRenamed("full_field_name", "des_field_long")
                .withColumnRenamed("geo_area_id", "id_geo_area")
...
try {
    ...
    df = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_aux_d_field"))

    df = transformationDfDimField(df)
    df = addRowDummy(df)
