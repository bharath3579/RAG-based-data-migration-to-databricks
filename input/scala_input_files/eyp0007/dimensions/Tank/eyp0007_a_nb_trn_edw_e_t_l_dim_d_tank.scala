...
val order = Seq("id_vers", "id_tank", "des_tank", "fec_start_date", "fec_end_date", "des_type_tank", "ind_max_vol", "ind_max_vol_safe_lim", "ind_min_vol", "ind_min_vol_safe_lim", "id_fcty_1", "fec_create_date", "fec_update_date")

val rowDummy = Row("-99", "-99", "Not available", java.sql.Timestamp.valueOf("1900-01-01 00:00:00"), null, null, null, null, null, null, null, null, null)

def transformationDfDimTank (df: DataFrame): DataFrame = {
    ...
    dfTrn = dfTrn.withColumnRenamed("object_id", "id_tank")
                    .withColumnRenamed("name", "des_tank")
                    ...
                    .withColumnRenamed("tank_type", "des_type_tank")
                    .withColumnRenamed("max_vol", "ind_max_vol")
                    .withColumnRenamed("max_vol_safe_limit", "ind_max_vol_safe_lim")
                    .withColumnRenamed("min_vol", "ind_min_vol")
                    .withColumnRenamed("min_vol_safe_limit", "ind_min_vol_safe_lim")
...
def addRowDummy(df: DataFrame): DataFrame = {
    ...
    val dfDummy = spark.createDataFrame(Seq(rowDummy).asJava, dfTrn.schema)
...
try {
    ...
    df = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_aux_d_tank")).withColumnRenamed("id_bu", "bu")
    df_ec_codes = getDataFromSource(sourcesFinal("am_eyp0007_trn_tb_aux_d_ec_codes")).withColumnRenamed("id_bu", "bu_ec_codes")

    df = df.as("tank").join(
                            df_ec_codes.filter(col("code_type") === "TANK_TYPE").drop("created_date", "last_updated_date").dropDuplicates("code_text", "bu_ec_codes").as("ec_code"), 
                            col("tank.tank_type") === col("ec_code.code") &&
                            col("tank.bu") === col("ec_code.bu_ec_codes"),
                            "left"
                            )
    df = df.drop("tank_type", "code_type", "code", "bu_ec_codes" )
    df = df.withColumnRenamed("code_text", "tank_type")


    df = transformationDfDimTank(df)
    df = addRowDummy(df)
